...
export default function Payment() {
        ...
        const options = {
          ...
          handler: async function (responce) {
            ...
            if (res?.data?.success) {
              ...
              navigate("/profile?id=" + res?.data?.data?._id)
            }
  ...
  return (
    <>
      <div className="sm:min-h-[84vh] bg-[#4a4a4a] rounded-xl overflow-hidden font-sans text-slate-200 max-w-full mx-auto border border-white/5 shadow-2xl sm:px-44 3xl:px-76 sm:mt-6">
        {/* ── Hero ── */}
        ...
        <div className="py-4 px-6">
          <p className="text-[16px] font-semibold tracking-widest text-yellow-400/70 mb-3 uppercase flex items-center gap-1.5">
          ...
          <div className="space-y-2">
            {[
              { number: "5267 3181 8797 5449", label: "Visa" },
              { number: "5267 3181 8797 5449", label: "Mastercard" },
            ].map((card) => (
              <div key={card.number} className="flex items-center justify-between bg-white/5 rounded-lg px-4 py-3">
                <div>
                  <p className="text-white text-sm font-mono">{card.number}</p>
                  <p className="text-white/35 text-xs mt-0.5">{card.label} · Any expiry · Any CVV · OTP: 1234</p>
                </div>
                <button
                  onClick={() => navigator.clipboard.writeText(card.number.replace(/\s/g, ""))}
                  className="text-white/30 hover:text-white transition cursor-pointer ml-4"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
            ...
            <div className="flex items-center justify-between bg-white/5 rounded-lg px-4 py-3">
              <div className="flex items-center justify-between bg-white/5 rounded-lg px-4 py-3">
                <div>
                  <p className="text-white text-sm font-mono">Netbanking</p>
                  <p className="text-white/35 text-xs mt-0.5">Select any bank → click Success on the next page</p>
                </div>
              </div>
              <button
      ...
      {paid && <div className="absolute inset-0 w-full sm:min-h-[86px] flex items-center justify-center bg-[#2a2a2a]/90">
        <div className="sm:max-w-[420px] sm:min-h-[40vh] bg-[#4a4a4a] rounded-xl overflow-hidden font-sans text-slate-200 mx-auto border border-white/5 sm:py-14 sm:px-12 text-center animate__animated animate__zoomIn animate__fast">
          <div className="w-18 h-18 rounded-full bg-[#22c55e]/10 flex items-center justify-center mx-auto mb-4.5">
            <svg
              className="w-14 h-14"
              fill="none"
              stroke="#22c55e"
          ...
          <p className="text-4xl font-bold mb-1.5 text-white">
            You're subscribed!
          </p>
